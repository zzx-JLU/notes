...
// 尚未完成的依赖 CompletableFuture 对象也将异常完成，
// 用此 CancellationException 导致的 CompletionException 完成
...
要获得程序启动的进程的更多信息，或者想了解计算机上正在运行的任何其他进程，可以使用`ProcessHandle`接口。有 4 种方式得到`ProcessHandle`对象：
