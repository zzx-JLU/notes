...
三引号可以跨行使用，并且会保留字符串中的格式（如换行等）。例如：
...
可以使用`list()`将其他序列转换成列表。通过这种方法可以将不可变序列转换成可变的列表，使用列表的操作修改序列后，在转换回原类型。
...
可以使用`dict()`来创建字典，每个参数都是一个键值对，参数名为键，参数值为值。用这种方式创建的字典，键都是字符串。例如：
...
`dict()`也可以将一个包含双值子序列的序列转换为字典，每个双值子序列转换为一个键值对，其中第一个元素为键，第二个元素为值。例如：
