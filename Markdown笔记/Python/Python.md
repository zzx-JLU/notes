...
可以使用`dict()`函数来创建字典，每个参数都是一个键值对，参数名为键，参数值为值。用这种方式创建的字典，键都是字符串。例如：
...
`dict()`函数也可以将一个包含双值子序列的序列转换为字典，每个双值子序列转换为一个键值对，其中第一个元素为键，第二个元素为值。例如：
...
使用空的大括号`{}`创建的是空字典，而不是空集合。要创建空集合，需要使用`set()`函数，例如：
...
使用`set()`函数将字典转换成集合时，只包含字典中的键，不包含值。
...
将可能出现异常的代码放到`try`语句中，如果没有异常则跳过`except`子句，正常执行其他代码；如果出现异常，则执行`except`子句中的代码。
