...
注意：输入的维度为 $d$，加上阈值的系数 $x_0=+1$，使得 $\boldsymbol{x}_i$ 为 $d+1$ 维向量，而样本个数恰好也为 $d+1$，因此矩阵 $X$ 为 $d+1$ 维方阵，而且是可逆矩阵。

如果对于任意一个输出 $\boldsymbol{y}=[y_1,y_2,\cdots,y_{d+1}]^{\text{T}}$，都存在权值 $\boldsymbol{w}$，使得 $\operatorname{sign}(X\boldsymbol{w})=\boldsymbol{y}$，则这组输入就是可被打散的。

要满足 $\operatorname{sign}(X\boldsymbol{w})=\boldsymbol{y}$，不妨令 $X\boldsymbol{w}=\boldsymbol{y}$，由于 $X$ 是可逆矩阵，可得 $\boldsymbol{w}=X^{-1}\boldsymbol{y}$。因此，任意一种输出都是可以达到的，输入 $X$ 是可以被打散的。
...
矩阵 $X$ 的行数为 $d+2$，列数为 $d+1$，也就是有 $d+2$ 个 $d+1$ 维行向量，则这 $d+2$ 个行向量线性相关，任意一个行向量都可以表示为其他行向量的线性组合。因此有
$$
...
假设输入可以被打散，就一定存在输出满足以下条件：当 $a_i>0$ 时，$\boldsymbol{w}^{\text{T}}\boldsymbol{x}_i>0$，$y_i=1$；当 $a_i<0$ 时，$\boldsymbol{w}^{\text{T}}\boldsymbol{x}_i<0$，$y_i=-1$。此时一定有 $\boldsymbol{w}^{\text{T}}\boldsymbol{x}_{d+2}>0$，$y_{d+2}$ 一定为 $+1$ 而不可能为 $-1$，有一种输出情况无法产生，与假设矛盾。因此，输入 $X$ 不可以被打散。

由 $X$ 的一般性可得，任意一个大小为 $d+2$ 的输入都不能被打散，因此 $d_{\text{VC}}\leqslant d+1$。
...
将损失函数的矩阵形式展开，得
$$
E_{\text{in}}(\boldsymbol{w})=\dfrac{1}{N}\begin{Vmatrix}
    X\boldsymbol{w}-\boldsymbol{y}
\end{Vmatrix}^2=\dfrac{1}{N}(\boldsymbol{w}^{\text{T}}X^{\text{T}}X\boldsymbol{w}-2\boldsymbol{w}^{\text{T}}X^{\text{T}}\boldsymbol{y}+\boldsymbol{y}^{\text{T}}\boldsymbol{y})
$$

可以证明，损失函数的梯度为
$$
\tag{3-3}
\nabla E_{\text{in}}(\boldsymbol{w})=\dfrac{2}{N}(X^{\text{T}}X\boldsymbol{w}-X^{\text{T}}\boldsymbol{y})
$$

令 $\nabla E_{\text{in}}(\boldsymbol{w})=\boldsymbol{0}$，当 $X^{\text{T}}X$ 可逆时，由式 $(3\text{-}3)$ 易得
$$
\tag{3-4}
\boldsymbol{w}_{\text{LIN}}=(X^{\text{T}}X)^{-1}X^{\text{T}}\boldsymbol{y}
$$

令 $(X^{\text{T}}X)^{-1}X^{\text{T}}=X^{\dag}$，称为 $X$ 的伪逆矩阵（pseudo-inverse），则
$$
\tag{3-5}
\boldsymbol{w}_{\text{LIN}}=X^{\dag}\boldsymbol{y}
$$

当 $X^{\text{T}}X$ 不可逆时，极小值点不唯一。通过改变 $X^{\dag}$ 的定义，可以使得式 $(3\text{-}5)$ 仍然成立，从而找到最小值点。

线性回归算法：

1. 构造输入矩阵 $X$ 和输出向量 $\boldsymbol{y}$。
2. 计算伪逆矩阵 $X^{\dag}$。
3. 输出结果 $\boldsymbol{w}_{\text{LIN}}=X^{\dag}\boldsymbol{y}$。

得到最优参数 $\boldsymbol{w}_{\text{LIN}}$ 后，就可以计算预测值 $\hat{y}=\boldsymbol{w}_{\text{LIN}}^{\text{T}}\boldsymbol{x}$。如果将所有预测值组织成向量，可得 $\hat{\boldsymbol{y}}=XX^{\dag}\boldsymbol{y}$。
...
\begin{aligned}
    ...
    \end{Vmatrix}^2\\
    &=\dfrac{1}{N}\begin{Vmatrix}
        \boldsymbol{y}-XX^{\dag}\boldsymbol{y}
    \end{Vmatrix}^2\\
    &=\dfrac{1}{N}\begin{Vmatrix}
        (I-XX^{\dag})\boldsymbol{y}
    \end{Vmatrix}^2
\end{aligned}
$$

将 $XX^{\dag}$ 称为帽子矩阵（hat matrix），记作 $H$。因为 $\hat{\boldsymbol{y}}=XX^{\dag}\boldsymbol{y}$，把 $XX^{\dag}$ 乘在 $\boldsymbol{y}$ 前面就会给 $\boldsymbol{y}$ 戴上帽子。
...
由于 $\hat{\boldsymbol{y}}=X\boldsymbol{w}_{\text{LIN}}$，输出向量的每个分量满足 $\hat{y_i}=\boldsymbol{x}_i^{\text{T}}\boldsymbol{w}_{\text{LIN}}$，是 $\boldsymbol{x}_i$ 的各分量的线性组合，因此 $\hat{\boldsymbol{y}}$ 位于 $X$ 张成的线性空间中。而 $\boldsymbol{y}$ 不一定位于 $X$ 张成的线性空间中，为了使 $\boldsymbol{y}-\hat{\boldsymbol{y}}$ 最小，应使得 $\boldsymbol{y}-\hat{\boldsymbol{y}}$ 与 $X$ 张成的线性空间垂直。

帽子矩阵的作用就是将 $\boldsymbol{y}$ 投影到 $X$ 张成的线性空间中，得到 $\hat{\boldsymbol{y}}$，此时 $\boldsymbol{y}-\hat{\boldsymbol{y}}$ 与 $X$ 张成的线性空间垂直，可以保证 $\boldsymbol{y}-\hat{\boldsymbol{y}}$ 最小。$I-H$ 是一个线性变换，作用在 $\boldsymbol{y}$ 上，将 $\boldsymbol{y}$ 转换成 $\boldsymbol{y}-\hat{\boldsymbol{y}}$。

由 $H$ 和 $I-H$ 的几何意义可以直观地得到下列结论：$H^2=H$，$(I-H)^2=I-H$。
...
$$
\begin{aligned}
    tr(I-H)&=tr(I_{N\times N})-tr(H)\\
    &=N-tr(XX^{\dag})\\
    &=N-tr(X(X^{\text{T}}X)^{-1}X^{\text{T}})\\
    &=N-tr(X^{\text{T}}X(X^{\text{T}}X)^{-1})\\
    &=N-tr(E_{(d+1)\times(d+1)})\\
    &=N-(d+1)
\end{aligned}
$$

实际训练时，数据集中存在噪声。在输出向量 $\boldsymbol{y}$ 中，有些分量从目标函数 $f$ 中产生，有些是噪声，如果用向量 $\boldsymbol{n}$ 表示噪声，则 $\boldsymbol{y}=f(X)+\boldsymbol{n}$，如图 3.2 所示。
...
从图中可知，对噪声使用 $I-H$ 进行变换也可以得到 $\boldsymbol{y}-\hat{\boldsymbol{y}}$，因此有
$$
\tag{3-7}
\begin{aligned}
    E_{\text{in}}(\boldsymbol{w}_{\text{LIN}})&=\dfrac{1}{N}\begin{Vmatrix}
