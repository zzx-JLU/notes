...
\begin{definition}
    对 $\mathcal{F}$ 中任一单调不减的事件序列 $F_1 \subseteq F_2 \subseteq \cdots \subseteq F_n \subseteq \cdots$，称 $\displaystyle\bigcup_{i=1}^{\infty} F_i$ 为 $\{F_n\}$ 的\textbf{极限事件}，记为
    $$
...
\begin{definition}
    对 $\mathcal{F}$ 上的一个概率 $P$，若它对 $\mathcal{F}$ 中任一单调不减的事件序列 $\{F_n\}$ 均有
    $$
...
\begin{property} \label{prop:连续性}
    若 $P$ 为事件域 $\mathcal{F}$ 上的概率，则 $P$ 既是下连续的，又是上连续的.
\end{property}
...
\begin{myproof}
    ...
    $$
    \begin{aligned}
        P(\lim_{n \to \infty} F_n) &= P(\bigcup_{i=1}^{\infty} F_i) \\
        &= P(\bigcup_{i=1}^{\infty} (F_i - F_{i-1})) \\
    ...
    由有限可加性得
    $$
    \sum_{i=1}^n P(F_i - F_{i-1}) = P(\bigcup_{i=1}^n (F_i - F_{i-1})) = P(F_n)
    $$
    所以
    $$
    ...
    \begin{aligned}
        ...
        &= P(\bigcup_{i=1}^{\infty} \overline{E_i}) \\
        &= P(\overline{\bigcap_{i=1}^{\infty} E_i}) \\
        &= 1 - P(\bigcap_{i=1}^{\infty} E_i) \\
...
\begin{myproof}
    由概率的有限可加性和连续性的证明过程可知必要性成立.下面证明充分性.

    设 $A_i \in \mathcal{F}, \, i=1,2,\cdots$ 是两两互不相容的事件序列，由有限可加性可知，对任意有限的 $n$ 都有
    $$
    P(\bigcup_{i=1}^n A_i) = \sum_{i=1}^n P(A_i) \leqslant 1
    $$
    因此正项级数 $\displaystyle\sum_{i=1}^{\infty} P(A_i)$ 收敛，即
    $$
    \lim_{n \to \infty} P(\bigcup_{i=1}^n A_i) = \lim_{n \to \infty} \sum_{i=1}^n P(A_i) = \sum_{i=1}^{\infty} P(A_i)
    $$
    记
    $$
    ...
    $$
    \lim_{n \to \infty} P(\bigcup_{i=1}^n A_i) = \lim_{n \to \infty} P(F_n) = P(\bigcup_{i=1}^{\infty} F_i) = P(\bigcup_{i=1}^{\infty} A_i)
    $$
    因此
    $$
    P(\bigcup_{i=1}^{\infty} A_i) = \sum_{i=1}^{\infty} P(A_i)
...
注意：
\begin{enumerate}
    \item 将条件改成“$A_1, A_2, \cdots, A_n$ 两两互不相容，且 $B \subseteq \displaystyle\bigcup_{i=1}^n A_i$”，全概率公式仍然成立.
    \item 将条件改成“$A_1, A_2, \cdots, A_n, \cdots$ 两两互不相容，且 $B \subseteq \displaystyle\bigcup_{i=1}^{\infty} A_i$”，全概率公式仍然成立.
\end{enumerate}
...
\begin{conclusion}
    若 $A,B,C$ 相互独立，则 $A \cup B$ 与 $C$ 相互独立，$AB$ 与 $C$ 相互独立，$A-B$ 与 $C$ 相互独立.
\end{conclusion}
