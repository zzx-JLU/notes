...
\begin{definition}
    ...
    $$
    则称随机变量 $X$ 服从参数为 $p$ 的{\heiti 0-1分布}或{\heiti 两点分布}（two-point distribution）.
\end{definition}
...
\begin{definition}
    ...
    \end{equation}
    称为{\heiti 贝塔函数}，也叫{\heiti 欧拉第一积分}，其中 $a>0, b>0$.
\end{definition}
...
\begin{definition}
    ...
    \begin{equation}
        ...
        \end{cases}
    \end{equation}
    则称 $X$ 服从{\heiti 贝塔分布}，记作 $X \sim Be(a,b)$，其中 $a>0, b>0$，$a,b$ 称为形状参数.
\end{definition}
...
\begin{definition}
    ...
    \end{equation}
    则称随机变量 $X$ 服从{\heiti 柯西分布}（Cauchy distribution），也称为{\heiti 柯西-洛伦兹分布}，记作 $X \sim C(\gamma, x_0)$，其中 $\gamma$ 称为{\heiti 尺度参数}，$x_0$ 称为{\heiti 位置参数}.
\end{definition}
