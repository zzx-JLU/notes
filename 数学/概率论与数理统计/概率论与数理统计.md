...
$$
\bigcup_{i=1}^n A_i=\{事件\;A_1,A_2,\cdots,A_n\;中至少有一个发生\}\\[0.5em]
...
$$
\bigcap_{i=1}^n A_i=\{事件\;A_1,A_2,\cdots,A_n\;同时发生\}\\[0.5em]
...
$$
P(\left(\bigcup_{i=1}^\infty B_i\right)|A)=\sum_{i=1}^\infty P(B_i|A)
$$
...
$$
\dfrac{\lambda^k e^{-\lambda}}{k!}>0\\[0.5em]
...
>
> $$
> 1\times\left(1-\dfrac{1}{n}\right) \left(1-\dfrac{2}{n}\right) \cdots \left(1-\dfrac{k-1}{n}\right) \to 1\\[0.5em]
> \left(1-\dfrac{\lambda}{n}\right)^n \to e^{-\lambda}\\[0.5em]
...
其中 $\lambda>0$ 是常数，则称 $X$ 服从参数为 $\lambda$ 的**指数分布**.
...
<div align="center" style="margin-bottom: 14px">
    ...
    <div style="width: 50%; margin-left: 50%; position: relative; top: 3.5px;">
...
$$
f_{X}(x)=\int_{-\infty}^{+\infty} f(x,y)\,\text{d}y,\ x\in\R\\[0.5em]
...
$$
f(x,y)=\dfrac{1}{2\pi\sigma_1 \sigma_2 \sqrt{1-\rho^2}}e^{\frac{-1}{2(1-\rho^2)} \left[\frac{(x-\mu_1)^2}{\sigma_1^2}-2\rho\frac{(x-\mu_1)(y-\mu_2)}{\sigma_1 \sigma_2}+\frac{(y-\mu_2)^2}{\sigma_2^2}\right]},\ (x,y)\in\R^2
$$
...
>
> $$
> f(\mu_1,\mu_2)=\dfrac{1}{2\pi\sigma_1 \sigma_2 \sqrt{1-\rho^2}}=\dfrac{1}{2\pi\sigma_1 \sigma_2}=f_{X}(\mu_1)\,f_{Y}(\mu_2)
> $$
>
> 从而 $\rho=0$.
...
\begin{aligned}
    ...
    &=\dfrac{\dfrac{\partial F(x,y)}{\partial y}}{\dfrac{\text{d}}{\text{d}y}F_{Y}(y)}\\[2em]
...
>     &=\sum_{i=0}^k \left[\dfrac{\lambda_1^l e^{-\lambda_1}}{l!}\dfrac{\lambda_2^{k-l} e^{-\lambda_2}}{(k-l)!}\right]\\
>     &=\sum_{i=0}^k \left[\dfrac{k!}{l!(k-l)!}\lambda_1^l \lambda_2^{k-l}\dfrac{e^{-(\lambda_1+\lambda_2)}}{k!}\right]\\
