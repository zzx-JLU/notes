...
\begin{definition}
    ...
    \end{cases}
    $$
    则称 $X$ 在区间 $(a,b)$ 上服从\textbf{均匀分布}，记作 $X \sim U(a,b)$.
\end{definition}
...
\begin{definition}
    ...
    \end{cases}
    $$
    其中 $\lambda>0$ 是常数，则称 $X$ 服从参数为 $\lambda$ 的\textbf{指数分布}.
\end{definition}
...
\begin{figure}[htbp]
    ...
    \subfigure[正态分布的概率密度]{
        ...
        \begin{tikzpicture}[>=Stealth, yscale=5]
            ...
            \node at (0, 0) [below left] {$O$};
            % 曲线
            \draw[domain=\Mu-3*\Sigma:\Mu+3*\Sigma, line width=1.5pt] plot (\x, {exp(-1 * pow(\x-\Mu, 2) / (2*\Sigma*\Sigma)) / (sqrt(2*pi)*\Sigma)});
            % 辅助线
    ...
    \subfigure[正态分布的分布函数]{
        ...
        \begin{tikzpicture}[>=Stealth, yscale=2]
            ...
            \node at (0, 0) [below left] {$O$};
            % 曲线
            \draw[domain=-0.5:4.5, line width=1.5pt] plot (\x, {1 / (1+exp(-1.70174454109 * (\x-\Mu)))});
            % 辅助线
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[>=Stealth, yscale=4]
        ...
        % 曲线
        \draw[domain=\Mu-3*\Sigma:\Mu+3*\Sigma, line width=1.5pt] plot (\x, {exp(-1 * pow(\x-\Mu, 2) / (2*\Sigma*\Sigma)) / (sqrt(2*pi)*\Sigma)});
        \draw[domain=\xMu-3*\Sigma:\xMu+3*\Sigma, line width=1.5pt] plot (\x, {exp(-1 * pow(\x-\xMu, 2) / (2*\Sigma*\Sigma)) / (sqrt(2*pi)*\Sigma)});
        % 辅助线
...
如果固定 $\mu$，改变 $\sigma$ 的值，则由 $y_{\text{max}}=\dfrac{1}{\sqrt{2\pi}\sigma}$ 可知，当 $\sigma$ 越小时概率密度曲线在 $x=\mu$ 附近越陡峭，$X$ 落在 $x=\mu$ 附近的概率越大；当 $\sigma$ 越大时概率密度曲线越平坦，如图 \ref{fig:sigma} 所示.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[>=Stealth, yscale=5]
        ...
        % 曲线
        \draw[domain=\Mu-3*\Sigma:\Mu+3*\Sigma, line width=1.5pt] plot (\x, {exp(-1 * pow(\x-\Mu, 2) / (2*\Sigma*\Sigma)) / (sqrt(2*pi)*\Sigma)});
        \draw[domain=\Mu-3*\sSigma:\Mu+3*\sSigma, line width=1.5pt, draw=red] plot (\x, {exp(-1 * pow(\x-\Mu, 2) / (2*\sSigma*\sSigma)) / (sqrt(2*pi)*\sSigma)});
        \draw[domain=\Mu-2*\tSigma:\Mu+2*\tSigma, line width=1.5pt, draw=blue] plot (\x, {exp(-1 * pow(\x-\Mu, 2) / (2*\tSigma*\tSigma)) / (sqrt(2*pi)*\tSigma)});
        % 辅助线
